...
function ClientsPage({clients,setClients,userRole,staff,contacts=[]}){
  ...
  const filteredContacts=useMemo(()=>{
    const db=(window as any).__DW_CONTACTS__||contacts||[];
  ...
  return(<div className="space-y-5">{confirmEl}
    ...
    <div className="flex items-center justify-between flex-wrap gap-3">
      <div className="flex gap-1 border border-gray-200 rounded-xl p-1 bg-white">
        ...
        <button onClick={()=>setTab("contacts")} className={`px-5 py-2 rounded-lg text-sm font-semibold transition-all ${tab==="contacts"?"text-white":"text-gray-500"}`} style={tab==="contacts"?{background:G}:{}}> All Contacts ({((window as any).__DW_CONTACTS__||contacts||[]).length})</button>
      </div>
    </div>
    ...
    {tab==="contacts"&&<div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs text-gray-400 whitespace-nowrap">Showing {filteredContacts.length} of {((window as any).__DW_CONTACTS__||contacts||[]).length}</span>
      </div>
      <Card>
