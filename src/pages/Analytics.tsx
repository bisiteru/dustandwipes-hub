...
function AnalyticsPage({clients,siteReports,jobs,staff,absences=[],requests=[]}){
  ...
  const techMap:Record<string,number>={};jobs.forEach((j:any)=>{(j.techs||"").split(",").map((t:string)=>t.trim()).filter(Boolean).forEach((t:string)=>{techMap[t]=(techMap[t]||0)+1;});});
  const techData=Object.entries(techMap).sort((a,b)=>b[1]-a[1]).slice(0,8).map(([name,count])=>({name,count}));
  const monthlyJobs=useMemo(()=>{const m:Record<string,number>={};jobs.forEach((j:any)=>{if(j.date){const k=j.date.slice(0,7);m[k]=(m[k]||0)+1;}});return Object.entries(m).sort((a,b)=>a[0].localeCompare(b[0])).slice(-6).map(([k,v])=>({month:k.slice(5)+" "+k.slice(0,4),count:v}));
},[jobs]);
  // Revenue trend: monthly contract totals from client start dates
  const revenueTrend=useMemo(()=>{const m:Record<string,number>={};clients.forEach((c:any)=>{if(c.cs){const k=c.cs.slice(0,7);m[k]=(m[k]||0)+(Number(c.tot)||0);}});return Object.entries(m).sort((a,b)=>a[0].localeCompare(b[0])).slice(-8).map(([k,v])=>({month:k.slice(5)+"/"+k.slice(2,4),revenue:v}));},[clients]);
  // Contract health
